...
class SIFTFlowSegDataLayer(caffe.Layer):
	...
	self.label_semantic = self.load_labelmap(self.indices[self.idx])
      #  self.label_geometric = self.load_label(self.indices[self.idx], label_type='geometric')
	self.prior = self.load_prior(self.indices[self.idx])
	#self.rare = self.load_rare(self.indices[self.idx])
        # reshape tops to fit (leading 1 is for batch dimension)
        top[0].reshape(1, *self.data.shape)
        top[1].reshape(1, *self.label_semantic.shape)
      #  top[2].reshape(1, *self.label_geometric.shape)
	top[2].reshape(1, *self.prior.shape)
	#top[4].reshape(1, *self.rare.shape)
	...
	top[2].data[...] = self.prior
	#top[4].data[...] = self.rare
    ...
    def	load_labelmap(self, idx):
	im = Image.open('{}/SemanticLabels/fixation_500/{}.jpg'.format(self.siftflow_dir, idx))
	in_ = np.array(im, dtype=np.float32)
	return in_

    def	load_rare(self, idx):
	im = Image.open('{}/Images/rare/{}.jpg'.format(self.siftflow_dir, idx))
	in_ = np.array(im, dtype=np.float32)
	in_ = in_-110
	return in_
    ...
    def load_label(self, idx, label_type=None):
        ...
        if label_type == 'semantic':
            label = scipy.io.loadmat('{}/SemanticLabels/spatial_envelope_256x256_static_8outdoorcategories/{}.mat'.format(self.siftflow_dir, idx))['fix']
        elif label_type == 'geometric':
            label = scipy.io.loadmat('{}/GeoLabels/spatial_envelope_256x256_static_8outdoorcategories/{}.mat'.format(self.siftflow_dir, idx))['fix']
            label[label == -1] = 0
        else:
	...
	prior=scipy.io.loadmat('{}/prior/prior.mat'.format(self.siftflow_dir))['p']

...
class SIFTFlowSegDataLayer(caffe.Layer):
	...
	im1 = Image.open('{}/Images/rare/{}.jpg'.format(self.siftflow_dir, idx))
        in_1 = np.array(im1, dtype=np.float32)
        in_ = np.array(im, dtype=np.float32)
        in_ = in_[:,:,::-1]
        in_ -= self.mean
	in_2=np.zeros((500,500,4),dtype=np.float32)
	in_2[:,:,0:3] = in_
	in_2[:,:,3]=in_1
        in_ = in_2.transpose((2,0,1))
        return in_
